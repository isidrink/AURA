...
const EMPTY_VISEME_WEIGHTS: Record<Viseme, number> = {
...
export default function Home() {
  ...
  const visemeWeightsRef = useRef<Record<Viseme, number>>({
    ...EMPTY_VISEME_WEIGHTS,
  });
  const headAudioRef = useRef<{
    stop: () => void;
    resetAll: () => void;
  } | null>(null);
  const headAudioInitRef = useRef<Promise<void> | null>(null);
  const headAudioFrameRef = useRef(0);
  ...
  const stopHeadAudio = useCallback(() => {
    cancelAnimationFrame(headAudioFrameRef.current);
    headAudioRef.current?.stop();
    headAudioRef.current?.resetAll();
    headAudioRef.current = null;
    headAudioInitRef.current = null;
    visemeWeightsRef.current = { ...EMPTY_VISEME_WEIGHTS };
  }, []);

  const setupHeadAudio = useCallback((analyser: AnalyserNode) => {
    if (headAudioRef.current || headAudioInitRef.current) return;
    headAudioInitRef.current = (async () => {
      try {
        const context = analyser.context;
        await context.audioWorklet.addModule("/headworklet.min.mjs");
        const { HeadAudio } = await import(
          "@met4citizen/headaudio/dist/headaudio.min.mjs"
        );
        const node = new HeadAudio(context, {
          parameterData: {
            vadMode: 1,
            vadGateActiveDb: -43,
            vadGateActiveMs: 8,
        ...
        await node.loadModel("/model-en-mixed.bin");
        node.onvalue = (key: string, value: number) => {
          const viseme = key.replace("viseme_", "") as Viseme;
          if (viseme in visemeWeightsRef.current) {
            visemeWeightsRef.current[viseme] = value;
          }
        };
        analyser.connect(node);
        node.start();
        headAudioRef.current = node;

        let previous = performance.now();
        const update = (now: number) => {
          node.update(Math.min(50, now - previous));
          previous = now;
          headAudioFrameRef.current = requestAnimationFrame(update);
        };
        headAudioFrameRef.current = requestAnimationFrame(update);
      } catch {
        headAudioInitRef.current = null;
  ...
  }, [active, locale, selectedPrompt, setupHeadAudio, stopCall, stopHeadAudio]);
